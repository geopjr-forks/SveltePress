<script context="module">
	export async function load({ page, fetch, session, context }) {
		const ls = await fetch('http://localhost:8080/ls', {
			method: 'POST',
			cache: 'no-cache',
...
<script>
	...
	async function list(dirPath, fullPath = false) {
		...
		ls = await fetch('http://localhost:8080/ls', {
			method: 'POST',
			cache: 'no-cache',
	...
	async function confirmWorkDir({ spuiPath }) {
		...
		const res = await fetch('http://localhost:8080/setworkdir', {
			method: 'POST',
			cache: 'no-cache',
	...
	$: safeProjectName = projectName.replace(/[^a-z0-9 -]/gi, '').replace(' ', '-');
	...
	async function runCommand() {
		...
		const res = await fetch('http://localhost:8080/runCommand', {
			method: 'POST',
			cache: 'no-cache',
